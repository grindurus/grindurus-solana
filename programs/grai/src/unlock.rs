...
/// (which goes to `treasury`). `grai_amount == 0` accrues (and optionally claims) without
/// unlocking.
///
/// Remaining accounts: quads `[asset_config, position, vault_ata, holder_ata]` per listed asset in
/// registry order. `asset_config` must be writable when `claim_all` is set.
pub fn execute_unlock<'info>(
    ...
    let (unlock_amount, penalty) = if grai_amount > 0 {
        preview_unlock(
            grai_amount,
            old_amount,
