...
| `npm run setSettlementAsset` | `grai/setSettlementAsset.ts` | Select the listed USDC settlement asset                        |
| `npm run setFeed`    | `grai/setFeed.ts`     | List USDC via `set_feed(paused, feed)`                     |
...
| `npm run allocate`        | `grinders/allocate.ts`         | Move asset from Grinders ATA → custodian wallet                  |
...
Reference for **Chainlink** and **Pyth** accounts used in `add_asset`, `set_feed`, `deposit`, `deposit_sol`, and `distribute`.
...
| ------------ | ---------------------------------------------- | ------------------------------------------------------------------ | -------------------------------- | --------------------------------------------- |
| **USDC/USD** | `Dpw1EAVrSB1ibxiDQyTAW6Zip3J4Btk2x4SgApQCeFbX` | `eaa020c61cc479712813461ce153894a96a6c00b21ed0cfc2798d1f9a9e9c94a` | `grai/setFeed.ts` | Verified on devnet 2026-06-26                 |
...
USDC_USD_PRICE_FEED=Dpw1EAVrSB1ibxiDQyTAW6Zip3J4Btk2x4SgApQCeFbX npm run setFeed

# USDC — Chainlink alternative (devnet only)
USDC_USD_PRICE_FEED=2EmfL3MqL3YHABudGNmajjCpR13NNEn9Y4LWxbDm6SwR npm run setFeed
```
